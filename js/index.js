/**
Issues:
have to hide the initial button, as twttr doesn't exist first time we try to remove it.
Hence the delay before we remove the initial (rubbish) button and create a new one
**/
$(document).ready( function(){
  ...
  function displayQuote( new_quote) {
    $('#quote-text').html( new_quote);
  }
  function pushTweetText(){
    ...
    var tweetBtn = $('<a></a>')
      ...
      .attr('data-text', current_quote); // $('#quote_text').val());
    $('#tweetBtn').append(tweetBtn);
    twttr.widgets.load();
  }
  $('.btn').click( function( e){
    current_quote = generateQuote();
    displayQuote( current_quote);
    pushTweetText();
  });
  // removed html static button, so we don't need this anymore
  // $('#tweetBtn').hide();
  // initialise quote
  current_quote = generateQuote();
  displayQuote( current_quote);
  setTimeout( pushTweetText, 300);
});
